...
class generator:
	...
	def gen(self,x,training):
		...
		layer_1 = conv2d(x,self.weights[scope+'w_conv1']) + self.biases[scope+'b_conv1']
		layer_1 = lrelu(layer_1)

		layer_2 = conv2d(layer_1,self.weights[scope+'w_conv2']) + self.biases[scope+'b_conv2']
		layer_2 = batch_norm(layer_2,training=training,name=scope+'conv2')
		layer_2 = prelu(layer_2,scope+'w_conv2')

		layer_3 = conv2d(layer_2,self.weights[scope+'w_conv3']) + self.biases[scope+'b_conv3']
		layer_3 = batch_norm(layer_3,training=training,name=scope+'conv3')
		layer_3 = prelu(layer_3,scope+'w_conv3')

		layer_4 = conv2d(layer_3,self.weights[scope+'w_conv4']) + self.biases[scope+'b_conv4']
		...
		if self.scope_name==1:
			...
			layer_5 = conv2d(layer_4,self.weights[scope+'w_conv5']) + self.biases[scope+'b_conv5']
			...
			layer_6 = conv2d(layer_5,self.weights[scope+'w_conv6']) + self.biases[scope+'b_conv6']
			...
			layer_7 = conv2d(layer_6,self.weights[scope+'w_conv7']) + self.biases[scope+'b_conv7']
			...
			layer_8 = conv2d(layer_7,self.weights[scope+'w_conv8']) + self.biases[scope+'b_conv8']
			...
			layer_9 = conv2d(layer_8,self.weights[scope+'w_conv9']) + self.biases[scope+'b_conv9']
			layer_9 = subpixel2d(layer_9,[-1,self.shape*2,self.shape*2,64])
			layer_9 = prelu(layer_9,scope+'w_conv9')
			layer = layer_9

		output = conv2d(layer,self.weights[scope+'out']) + self.biases[scope+'out']
